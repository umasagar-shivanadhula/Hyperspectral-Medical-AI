...
const HSI_API = {
  async predictPerfusion(file, rawFile = null) {
    const formData = new FormData();
    formData.append('file', file);
    if (rawFile) formData.append('raw_file', rawFile);
  ...
  // Local path — for large .b2nd files (27-43 GB) that can't be uploaded
  async predictPerfusionLocal(localPath) {
    const resp = await fetch(`${API_BASE}/predict/perfusion/local`, {
      method: 'POST',
  ...
  async predictTumor(file, rawFile = null) {
    const formData = new FormData();
    formData.append('file', file);
    if (rawFile) formData.append('raw_file', rawFile);
  ...
  async predictTumorLocal(localPath) {
    const resp = await fetch(`${API_BASE}/predict/tumor/local`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ path: localPath })
    });
