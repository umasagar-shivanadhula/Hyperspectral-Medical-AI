...
def load_b2nd_cube(path: Union[str, Path]) -> np.ndarray:
    ...
    Returns:
        Float32 NumPy array with shape (H, W, B).
        - If the stored array is 2D, adds a singleton band dimension (H, W, 1).
        - If the stored array is 3D and appears to be (bands, H, W),
          transposes it to (H, W, bands).

    Raises:
        ValueError if the stored array is not 2D or 3D.
    """
    path = Path(path)
